...
export const loadReviews = async function () {
    ReactDOM.render(ReactHtmlParser(generateReviews(review_data)), document.getElementById('reviews-parent6'));
}
...
export const composeReview = async function() {
    let bodyText = document.getElementById("textArea6").value.toString();
...
function Spicy9() {
    return (
        <div>
            ...
            <Container>
                <Tile isAncestor>
                    <Tile isParent isVertical isSize={12} id="reviews-parent6" dangerouslySetInnerHTML={{__html: generateReviews(review_data)}}>
