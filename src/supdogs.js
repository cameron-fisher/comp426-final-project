...
const review_data = require('./data/supReviews.json');

export const loadReviews = async function () {
    ReactDOM.render(ReactHtmlParser(generateReviews(review_data)), document.getElementById('reviews-parent7'));
}
...
export const composeReview = async function() {
    let bodyText = document.getElementById("textArea7").value.toString();
    //let bodyText = "Test to see if this is the problem";

    //let bodyText = "test text";
   await axios({
       method: 'post',
       url: "http://localhost:5000/supReviews/",
       data: {
           "body": bodyText
       }
   });
...
function SupDogs() {
    return (
        <div>
            ...
            <Box hasTextAlign='centered' className="is-shadowless">
                <Subtitle isSize={5}>Submit your thoughts and refresh to leave a review for Sup Dogs!</Subtitle>
                <Input type="text" id="textArea7" placeholder='' isSize='large'/>
                <Button onClick={submitClicked} isColor='primary' id="sendreview7" className="mt-4 mr-3">Submit</Button>
            ...
            <Container>
                <Tile isAncestor>
                    <Tile isParent isVertical isSize={12} id="reviews-parent7" dangerouslySetInnerHTML={{__html: generateReviews(review_data)}}>
